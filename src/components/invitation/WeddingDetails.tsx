...
/**
 * SCENE — the celebrations, following the reference video's "Timeline" panel:
 * a gold rail down the side, a medallion at each event, then day, date and time.
 * Events come from wedding.ts, so adding a function is one entry in the config.
 */
      ...
      <div className="mx-auto w-full max-w-2xl">
        ...
        <ol className="mt-14 space-y-12">
          {wedding.events.map((event, i) => (
            <Reveal key={event.name} delay={0.12 + i * 0.12} as="li">
              <div className="relative pl-12 sm:pl-16">
                ...
                <h3 className="display-name mt-3 text-3xl text-ink sm:text-4xl">{event.name}</h3>
